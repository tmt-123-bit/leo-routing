...
- `mappo_design.py`：共享候选打分 Actor、图注意力 centralized critic、running statistics、terminal/truncation GAE、安全 normalization 和 minibatch 工具。
- `leo_multiagent_env.py`：24 颗卫星对应 24 个 Agent，加入持续到达、三类业务、HOL 等待时间、deadline、正确的 route-switch 语义和固定 slot 事件顺序。
- `cleanmarl_leo_multiagent_wrapper.py`：`leo_multi` 接口输出 `(24,182)` Actor observation、`(24,7)` action mask 和 6941 维图状态传输向量。
- `cleanmarl_mappo_leo.py`：加入 shared candidate scorer、graph critic、active/no-route policy mask、PPO minibatch、梯度裁剪、KL/entropy/gradient/explained-variance 诊断、固定 validation split 和 checkpoint 选择。
- `run_exp004_mappo.py` / `run_exp005_diagnostics.py`：多 seed 训练、held-out 评测、层次 bootstrap、配对检验和训练诊断。
- `tle_topology_builder.py` / `run_tle_replay.py`：冻结 TLE、SGP4 动态拓扑生成和 zero-shot 回放。
- `test_mappo_design.py`：22 项环境、表示、图网络、PPO 和 TLE 测试。

本机结果：22 项测试全部通过；3 个 policy seed、2 个场景的 quick EXP-004 和 EXP-005 已跑通。300-step quick run 中 MAPPO 仍未超过 full heuristic，因此当前不能声称性能提升。完整 50,000-step 配置已经写入 runner 的 `full` 模式，但尚未在本轮 CPU 会话中跑完。
...
| `leo_marl_env.py` | Python MARL 环境接口，提供 `reset/step/observation/action_mask/reward/info`，之后接 MAPPO、CleanMARL 或其他成熟算法库。 |
...
Python 这块不是另起一个算法，而是给后面接 MAPPO/CTDE 留环境接口。
...
这块是为后面接 MAPPO/CTDE 做准备。现在代码还没有训练 Actor，只是把 Actor 需要的局部观测先构造出来。
...
该阶段不作为最终训练算法，而作为之后接 MAPPO、CleanMARL 或其他成熟 MARL 算法库的建模基础。
```

## Python 环境接口与文稿对齐

`leo_marl_env.py` 是之后接 MAPPO/CTDE 的环境层，不是新的独立算法。它对应文稿里的“实时 Dec-POMDP + CTDE/MAPPO”路线：执行时只输出当前卫星的一跳邻居观测和动作掩码，训练时通过 `global_state_for_critic` 提供集中式 Critic 可用的全局摘要。
...
我之前上传的 `Stable Hierarchical Routing for Operational LEO Networks` / SHORT 笔记对本项目主要有四点启发：第一，真实 LEO 网络不能只看扁平全局路由，控制开销和路由更新次数很关键；第二，可以引入 shell/orbit/geographic 层次，把快速变化的卫星运动和相对稳定的地理目标解耦；第三，每跳决策应尽量保持近 O(1)，只比较少量邻居；第四，实验指标除了平均时延，还要看控制开销、可用性、重路由/切换、CPU/内存或推理开销。

`leo_marl_env.py` 仍保留 toy `(U,W)` 位置编码，但动态链路已经可以由 `topology_provider(t, env)` 替换。当前 `tle_topology_builder.py` 已用冻结 Starlink TLE 和 SGP4 生成 24 星、30 时隙的连通链路快照；位置与传播时延来自 TLE，ISL 选择、容量和可靠性仍是实验假设。

## 重点参考仓库

之后接学习算法时，优先参考这些成熟实现，避免从零填算法坑：MAPPO 官方实现 `marlbenchmark/on-policy`，轻量实现 `AmineAndam04/cleanmarl`，较重但完整的 `facebookresearch/BenchMARL`。我还给过 `tmt-123-bit/leo-routing` 作为 PS-MAPPO queue-aware 和 link-lifetime constrained LEO routing 的研究笔记参考；现在环境无法直接访问 GitHub 页面，所以这里只先留作参考入口，不硬写没有确认过的仓库结构。

## Python 批量评测入口

`run_python_experiments.py` 是论文级实验前的统一评测入口。它还不是最后的 MAPPO 训练脚本，而是先把环境、场景、策略和指标导出流程固定下来，方便后续把训练好的 MAPPO Actor 接入同一个评测接口。
...
现在评测场景包括：`low_load`、`medium_load`、`hotspot_high_load`、`frequent_break`、`fault_links`。当前启发式策略包括：`random_feasible`、`delay_only`、`queue_load`、`full_masked_heuristic`。输出文件为：
...
注意：当前 `python_policy_eval_metrics.csv` 仍然是启发式策略和轻量学习策略的评测结果，还不是最后的 MAPPO 训练结果，也还不是最后的 IEEE 投稿实验。它的作用是先把评测闭环打通。之后要达到论文级结果，还需要接入 MAPPO/CleanMARL 训练、真实/准真实星座拓扑、Q-routing/普通 MAPPO 等学习型对比方法，并扩大 episode 数和随机种子数量。

## 轻量学习型 baseline

`train_linear_policy.py` 是一个无 PyTorch 依赖的轻量学习型 baseline。现在环境不能直接使用 PyTorch/Gym，也不能稳定联网安装依赖，所以这里先用 numpy 实现 masked linear softmax policy，并用 REINFORCE-style 更新跑通“学习型策略训练 -> 保存权重 -> 统一评测”的闭环。
...
注意：`linear_policy` 还不是最后的 MAPPO，也不是 IEEE 级最终算法。它的作用是证明现在环境已经支持学习型策略闭环，并为之后接 `marlbenchmark/on-policy`、`cleanmarl` 或 `BenchMARL` 提供接口参照。最终论文实验仍应接入正式 MAPPO/CTDE 或其他成熟 MARL 实现，并扩大随机种子、episode 数、星座规模和真实拓扑。
...
上面是单包 baseline 的接口记录。卫星级 `leo_multi` 使用 24 Agent、每 Agent 182 维 observation、7 个动作和按图解析的 6941 维 centralized state。当前已完成 3 个 policy seed 的 quick held-out 实验；结果只验证实验闭环，并未超过 full heuristic。

## 算法仓库接入建议

现在已经找到 `F:\cleanmarl`、`F:\on-policy`、`F:\BenchMARL` 三个算法仓库。综合代码复杂度、依赖和现在环境约束，现阶段最适合先接的是 `cleanmarl`，因为它的 `cleanmarl/mappo.py` 环境接口最直接，现在这版已经新增了 `cleanmarl_leo_wrapper.py` 来对齐它需要的 `reset/step/get_avail_actions/get_state/get_obs_size/get_state_size/get_action_size/n_agents` 接口。
...
这个文件把现在这版已经完成的 MATLAB/Python/评测/学习型 baseline/cleanmarl 桥接工作，和仍未完成的正式 MAPPO 训练、真实星座、论文级完整对比，整理成了一份清单。
...
tle_topology_builder.py
run_tle_replay.py
data/starlink_24_selected.tle
data/starlink_24_links.csv
```

其中：

- `real_topology_provider_example.py` 定义了 `topology_provider(t, env)` 需要返回的 `LinkState` 字典格式；
- `REAL_TOPOLOGY_INTEGRATION_PLAN.md` 说明了后续如何把 Hypatia、StarryNet、TLE/SGP4 等外部拓扑数据映射到现在环境，而不重写训练和评测逻辑。
- `tle_topology_builder.py` 使用 SGP4 生成链路时隙，并在写出前检查每个时隙连通；`run_tle_replay.py` 使用独立 workload seed 做 topology-transfer 回放。
...
它的作用是：如果后续定位到 `tmt-123-bit/leo-routing` 本地仓库，不需要推翻现在这版，而是按“主参考设计源”的方式，把它的状态设计、reward、mask、训练脚本思路和实验组织方式逐项并入当前工程实现。
