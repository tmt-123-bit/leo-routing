#!/usr/bin/env bash
# =============================================================================
# Experiment suite — reproduction check + full re-run with the optimized
...
# writes under experiments/<tag>/ so old results are never overwritten.
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")"
...
import csv,sys
try:
    rows=[r for r in csv.DictReader(open("experiments/archive/repro-check/aggregate_metrics.csv",encoding="utf-8-sig"))
...
# -----------------------------------------------------------------------------
if [[ "$STAGE" == "all" ]]; then
  echo "### STEP 2 FULL headline suite -> experiments/archive/EXP-004-FULL"
  run_exp004 --mode full --output experiments/archive/EXP-004-FULL
fi
...
if [[ "$STAGE" == "budget" || "$STAGE" == "all" ]]; then
  for M in x2k x10k; do
    echo "### STEP 4 budget sweep --mode $M -> experiments/qrouting-budget-$M"
    run_exp004 --mode "$M" --output "experiments/qrouting-budget-$M"
  done
  echo "### budget sweep done — plot delivery_ratio_mean vs {quick,x2k,x10k,full}"
  echo "    per scenario from each experiments/qrouting-budget-*/aggregate_metrics.csv"
fi
...
if [[ "$STAGE" == "mde" || "$STAGE" == "all" ]]; then
  FULL="experiments/archive/EXP-004-FULL/episode_metrics.csv"
  if [[ -f "$FULL" ]]; then
    echo "### STEP 5 MDE report -> experiments/archive/EXP-004-FULL/mde_report.csv"
