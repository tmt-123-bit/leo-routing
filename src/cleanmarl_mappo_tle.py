...
if __name__ == "__main__":
    ...
    kwargs = {}
    if args.env_type in {"leo", "leo_multi", "leo_tle"}:
        kwargs["project_path"] = args.leo_project_path
        kwargs["seed"] = args.seed
        kwargs["variant"] = args.leo_variant
        if args.env_type == "leo_tle":
            if not args.leo_topology_csv:
                raise ValueError("env_type=leo_tle requires --leo-topology-csv")
            kwargs["topology_csv"] = args.leo_topology_csv
    env = environment(
    ...
    candidate_feature_dim = None
    if args.env_type in {"leo", "leo_multi", "leo_tle"} or args.candidate_shared_actor:
    ...
    with open(os.path.join(checkpoint_root, "run_config.json"), "w", encoding="utf-8") as file:
    ...
    while step < args.total_timesteps:
        ...
        for epoch_idx in range(args.epochs):
            epoch_kls = []
            for transition_indices in shuffled_transition_minibatches(
                ...
                actor_loss = -pg_loss - args.entropy_coef * entropy
                ...
                critic_loss = F.mse_loss(value_prediction, value_target)
        ...
        if update_round % max(1, args.eval_steps) == 0:
            ...
            if args.env_type == "smaclite":
            ...
            elif args.env_type in {"leo", "leo_multi", "leo_tle"}:
                mean_delivery = float(
                    np.mean(
                ...
                validation_record = {
                    ...
                    "is_validation_best": validation_score == best_validation_score,
                }
                with open(metrics_path, "a", encoding="utf-8") as file:
                    file.write(json.dumps(validation_record, ensure_ascii=False) + "\n")

    final_checkpoint = save_checkpoint("final", step)
        ...
        json.dump(
            {
                ...
                ) if best_validation_score is not None else None,
