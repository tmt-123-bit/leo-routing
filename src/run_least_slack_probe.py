"""Single-change development probe: least-slack-first queue ordering on top of the
infeasibility purge.

Arm ``purge`` replicates the infeasible-drop intervention. Arm ``purge_ls``
additionally reorders every queue at slot boundaries so the head-of-line packet
is the one with the least scheduling slack, where slack is the remaining
deliverable slots minus the optimistic BFS hop distance to destination
(earliest-deadline-style urgency; stable packet-ID tie-break). The reorder is
...
class LeastSlackEnv(PurgeInfeasibleEnv):
    """Purge certainly-infeasible packets, then order queues by least slack."""
        ...
        for sat in range(1, self.n_agents + 1):
            ...
            if self.reorder_queues:
                def slack(packet_id):
                    packet = self.packets[packet_id]
                    deadline = self.cfg.packet_class_deadlines[packet.traffic_class]
                    effective_bound = min(packet.created_slot + deadline - 1, self.cfg.episode_slots)
                    dist = distances[packet.dst].get(sat)
                    dist = dist if dist is not None else 10 ** 6
                    return ((effective_bound - self.slot) - dist, packet_id)
                self.queues[sat] = deque(sorted(kept, key=slack))
            else:
...
def assess(rows):
    ...
    for scenario in LOADS:
        for method in BASE_METHODS:
            ...
            for treatment in ("purge", "purge_ls"):
...
def main():
    ...
    write_json(output / "manifest.json", {
        ...
        "intervention": (
            "arm purge replicates the infeasibility purge; arm purge_ls additionally "
            "reorders each queue at slot boundaries by least scheduling slack "
