#!/bin/bash
# Dev-tier training for the packet-management formal study: medium scenario,
# 8 seeds, SRPF environment (LEO_PURGE_INFEASIBLE=2). Sequential, ~21 min/seed.
cd /f/leo-routing-preliminary-matlab/src || exit 1
for SEED in 179055553 183895110 310818925 515636025 626669596 746965870 985998595 2136406109; do
  echo "=== training seed $SEED medium (purge+SRPF env) ==="
  LEO_PURGE_INFEASIBLE=2 /f/leo-venv/Scripts/python.exe /f/cleanmarl/cleanmarl/mappo.py \
    --env-type leo_multi --env-name medium_load \
    ...
    --checkpoint-dir "../outputs/dev-srpf-train-medium-20260917/seed_$SEED" \
    --run-tag DEV-SRPF-ENV-CONSTRAINED-MEDIUM-v1 \
