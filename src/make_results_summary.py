...
# 1 HEADLINE
rows = load("experiments/train-main/aggregate_metrics.csv")
c = defaultdict(dict)
for r in rows:
...
# 2 ABLATION
abrows = load("experiments/ablation/paired_ablation_effects.csv")
if abrows:
...
# 3 SCALE
rows = load("experiments/archive/scale-nolife/aggregate_scale.csv")
if rows:
    c3 = defaultdict(dict)
    for r in rows:
        c3[r["target_scale"]][(r["policy"], r["metric"])] = float(r["mean"])
    rci = ratio_ci("archive/scale-nolife")
...
# 4 REALISM
rows = load("experiments/archive/realism-nolife/aggregate_realism.csv")
if rows:
    c4 = defaultdict(dict)
    for r in rows:
        c4[r["topology"]][(r["policy"], r["metric"])] = float(r["mean"])
    rci = ratio_ci("archive/realism-nolife")
...
# 5 C3
rows = load("experiments/archive/TLE-NOLIFE-FULL/aggregate_metrics.csv")
if rows:
    c5 = defaultdict(dict)
    for r in rows:
...
# 6 LOAD
rows = load("experiments/archive/load-nolife/aggregate_load.csv")
if rows:
    c6 = defaultdict(dict)
    for r in rows:
        c6[r["load_point"]][(r["policy"], r["metric"])] = float(r["mean"])
    rci = ratio_ci("archive/load-nolife")
...
# 7 FAULT
rows = load("experiments/archive/fault-nolife/aggregate_fault.csv")
if rows:
    c7 = defaultdict(dict)
    for r in rows:
    ...
    rci = {}
    for r in load("experiments/archive/fault-nolife/paired_stats_ratio_ci.csv"):
...
out += ["", "## 8. Fairness (mechanism)", "",
        ...
        "|---|---|---|---|---|"]
srows = load("experiments/sweep-reward/summary.csv")
