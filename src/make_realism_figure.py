...
Three categories on the x-axis (training distribution -> real topologies):
  synthetic_24   (the 4x6 grid the policy was trained on; from archive/repro-check)
...
Usage:
  python make_realism_figure.py \
      --realism experiments/archive/realism-old/aggregate_realism.csv \
      --headline experiments/archive/repro-check/aggregate_metrics.csv \
      --outdir figures
"""
...
def main():
    ...
    ap.add_argument("--realism", default="experiments/archive/realism-old/aggregate_realism.csv")
    ap.add_argument("--headline", default="experiments/archive/repro-check/aggregate_metrics.csv")
