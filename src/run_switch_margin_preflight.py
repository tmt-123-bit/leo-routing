"""Run the frozen v4 greedy-margin MAPPO diagnostic preflight."""
...
SCREEN_NAME = "SWITCH-MARGIN-PREFLIGHT-v4"
SCHEMA_VERSION = 4
...
VALIDATION_WORKLOAD_SEEDS = tuple(range(46001, 46011))
TEST_WORKLOAD_SEEDS = tuple(range(47001, 47026))
...
PAIRED_ANALYSIS = "diagnostic_switch_margin_preflight_v4"
POLICY_NAMES = {
    **base.POLICY_NAMES,
    v1.ARM_INTEGRATED: "mappo_context_switch_margin_v4_beta_0p20_coef_0p01_margin_0p01",
}
...
SWITCH_REGULARIZER_FREEZE_RATIONALE = (
    "greedy margin 0.01 is the smallest frozen positive tie-breaking margin; "
    "coefficient 0.01 is the first predeclared scale-grid value whose late-probe "
    "regularizer-to-primary loss ratios fell inside [0.25, 0.75] in both "
    "scenarios, without consulting delivery or switch outcomes"
)
...
_BASE_OVERRIDES = {
    ...
    "PROTOCOL_FILENAME": "SWITCH_MARGIN_PREFLIGHT_V4.md",
    "SUMMARY_TITLE": "Switch-Margin MAPPO Preflight v4",
    "DEFAULT_OUTPUT_DIRECTORY_NAME": "switch-margin-preflight-6k-v4",
    "PARSER_DESCRIPTION": (
        "Run the frozen 6k-step greedy-margin MAPPO diagnostic preflight."
    ),
    ...
    "EXPECTED_ACTOR_SPEC": EXPECTED_ACTOR_SPEC,
    "EXPECTED_SWITCH_REGULARIZER_SPEC": EXPECTED_SWITCH_REGULARIZER_SPEC,
    "EXPECTED_EXTRA_RUN_CONFIG": {"log_loss_component_gradients": True},
...
@contextmanager
def configured_v4() -> Iterator[None]:
