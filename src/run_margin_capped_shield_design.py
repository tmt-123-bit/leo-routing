"""Design the relative-anchor margin-capped shield on the exposed v7 panel."""
...
import run_calm_selective_shield_design as v9
...
SCREEN_NAME = "MARGIN-CAPPED-SHIELD-DESIGN-v10"
SCHEMA_VERSION = 1
...
DESIGN_GRID = tuple(
    ...
    for index, margin_cap in enumerate(MARGIN_CAPS)
)
CELL_GATES = dict(v7.CELL_GATES)
RAW_SWITCH_HEADROOM_BUFFER = v9.RAW_SWITCH_HEADROOM_BUFFER
AVOIDABLE_MICRO_IMPROVEMENT_BUFFER = v9.AVOIDABLE_MICRO_IMPROVEMENT_BUFFER
DELIVERY_SLACK_BUFFER = v9.DELIVERY_SLACK_BUFFER
SELECTION_RULE_ID = (
    "max_worst_delivery_buffer_then_class2_then_raw_buffer_then_avoidable_"
    "buffer_then_max_margin_cap_index_v1"
)
EXPECTED_FEATURE_SCHEMA = candidate_feature_schema()
EXPECTED_FEATURE_NAMES = tuple(BASE_CANDIDATE_FEATURE_NAMES)
REFERENCE_POLICIES = dict(v9.REFERENCE_POLICIES)
BASELINE_FINGERPRINT_FILES = dict(v9.BASELINE_FINGERPRINT_FILES)
...
EXPECTED_V9_EPISODE_SHA256 = (
    "8dd6c0465141c28f05d1452775c0d76c9d67267b413f758dc823c9469f77889f"
)
EXPECTED_V9_GATE_SHA256 = (
    "92d96364a9f1573bf224203ce1e2f0a87cc86ca6e32f52d2ab97068236530368"
)
...
HISTORICAL_V10_RUNNER_FINGERPRINTS = {
    "runner": "22785166023882035728679219b9304af2108d509de220ac9852bc90e5959835",
...
def candidate_id(candidate: Mapping[str, Any]) -> str:
    return "_".join(
        (
            ...
            f"t{_token(float(candidate['margin_cap']))}",
        )
    )


def candidate_policy_name(candidate: Mapping[str, Any]) -> str:
    return f"mappo_proposed_margin_capped_{candidate_id(candidate)}_design_v10"
        ...
        return {
            **asdict(self),
            ...
            "execution_source": (
                ...
                else "new_v10_evaluation"
            ),
            "v9_source_candidate_index": (
                V9_REUSE_CANDIDATE_INDEX
                if self.candidate_index == 0
                else None
            ),
        }
            ...
            for policy_seed in POLICY_SEEDS:
                jobs.append(
                    DesignJob(
                        ...
                        margin_cap=float(candidate["margin_cap"]),
                    )
                )
    return jobs
...
def build_new_evaluation_jobs() -> list[DesignJob]:
    return [job for job in build_design_jobs() if job.candidate_index != 0]
...
def _runtime_code_fingerprint(project: Path, protocol: Path) -> dict[str, str]:
    files = {
        ...
        "protocol": protocol,
        "margin_capped_policy": project / "margin_capped_hysteresis_policy.py",
    ...
    if missing:
        raise FileNotFoundError(f"v10 fingerprint files missing: {missing}")
    modules = {
        "margin_capped_hysteresis_policy": "margin_capped_hysteresis_policy.py",
        ...
        "run_calm_selective_shield_design": (
            "run_calm_selective_shield_design.py"
        ),
    }
...
def _source_runtime_equivalence_record(
    ...
    if self_hash != EXPECTED_SOURCE_RUNTIME_EQUIVALENCE_SHA256:
    ...
    if (
        artifact.get("dynamic_contract", {}).get("feature_contract")
        != runtime_equivalence.EXPECTED_CURRENT_FEATURE_CONTRACT
    ):
...
def audit_v7_baseline(
    ...
    return v9.audit_v7_baseline(v7_output, source, project)


def _load_v9_new_shards(
    v9_output: Path,
    ...
    for job in v9.build_new_evaluation_jobs():
        csv_path, metadata_path = v9._evaluation_paths(v9_output, job)
        if not csv_path.is_file() or not metadata_path.is_file():
            raise FileNotFoundError(f"v9 new shard missing: {job.job_id}")
        checkpoint = source["checkpoints"][job.source_job_id]
        expected = v9._expected_shard_metadata(job, spec, checkpoint)
        metadata = source_runner._load_json(metadata_path)
        if set(metadata) != set(expected) | expected_metadata_fields:
            raise ValueError(f"v9 shard metadata field set mismatch: {job.job_id}")
        ...
        if metadata.get("csv_sha256") != csv_sha:
            raise ValueError(f"v9 shard CSV hash mismatch: {job.job_id}")
    ...
    jobs = v9.build_new_evaluation_jobs()
    expected_rows = len(jobs) * len(DESIGN_WORKLOAD_SEEDS)
    keys = {
        (row.scenario, row.policy, row.policy_seed, row.workload_seed)
        for row in rows
    }
    if len(rows) != expected_rows or len(keys) != expected_rows:
        raise ValueError("v9 new shard grid is incomplete or duplicated")
    if len(provenance) != 56:
        raise ValueError("v9 new shard audit did not cover all 56 jobs")
    return rows, provenance


def audit_v9_failure_and_shards(
    v9_output: Path,
    source: Mapping[str, Any],
    baseline: Mapping[str, Any],
) -> tuple[dict[str, Any], list[EpisodeMetrics]]:
    v9_output = v9_output.resolve()
    selection_path = v9_output / "design_selection.json"
    spec_path = v9_output / "calm_selective_shield_design_spec.json"
    episode_path = v9_output / "design_episode_metrics.csv"
    gates_path = v9_output / "design_cell_gates.csv"
    expected_file_hashes = {
        selection_path: EXPECTED_V9_SELECTION_FILE_SHA256,
        spec_path: EXPECTED_V9_SPEC_FILE_SHA256,
        episode_path: EXPECTED_V9_EPISODE_SHA256,
        gates_path: EXPECTED_V9_GATE_SHA256,
    }
    for path, expected_sha in expected_file_hashes.items():
        if not path.is_file() or sha256_file(path) != expected_sha:
            raise ValueError(f"frozen v9 artifact changed: {path.name}")

    selection = v9.load_design_selection(
        selection_path,
        allow_historical_runtime=True,
    )
    if selection.get("design_selection_sha256") != EXPECTED_V9_SELECTION_SHA256:
        raise ValueError("unexpected v9 selection self-hash")
    if selection.get("selection_status") != "no_eligible_candidate":
        raise ValueError("v10 requires the recorded failed v9 selection")
    if selection.get("eligible_candidate_indices") != []:
        raise ValueError("v9 failure unexpectedly contains eligible candidates")
    if selection.get("selected_candidate_index") is not None:
        raise ValueError("v9 failure unexpectedly selected a candidate")
    if selection.get("selected_parameters") is not None:
        raise ValueError("v9 failure unexpectedly selected parameters")
    for field in (
        "all_cells_pass",
        ...
        if selection.get(field) is not False:
            raise ValueError(f"v9 failure must set {field}=false")

    spec = source_runner._load_json(spec_path)
    if _self_hash(spec, "design_spec_sha256") != EXPECTED_V9_SPEC_SHA256:
        raise ValueError("unexpected v9 spec self-hash")
    expected_baseline = {
        key: baseline[key]
        for key in (
            "spec_sha256",
    ...
    if spec.get("v7_baseline") != expected_baseline:
        raise ValueError("v9 spec v7 baseline binding changed")

    v8_output = Path(str(spec["paths"]["v8_output"])).resolve()
    v8_reuse_record, v8_control_rows = v9.audit_v8_control_reuse(
        v8_output, source
    )
    if spec.get("v8_control_reuse") != v8_reuse_record:
        raise ValueError("v9 v8-control reuse record changed")
    new_rows, shard_index = _load_v9_new_shards(v9_output, spec, source)
    ...
    if combined_rows != reconstructed:
        raise ValueError("v9 combined rows do not reconstruct from all shards")

    source_policy = v9.candidate_policy_name(
        v9.DESIGN_GRID[V9_REUSE_CANDIDATE_INDEX]
    )
    candidate_rows = [row for row in new_rows if row.policy == source_policy]
    expected_candidate_rows = (
        len(SCENARIOS) * len(POLICY_SEEDS) * len(DESIGN_WORKLOAD_SEEDS)
    )
    candidate_keys = {
        (row.scenario, row.policy_seed, row.workload_seed)
        for row in candidate_rows
    }
    if (
        len(candidate_rows) != expected_candidate_rows
        or len(candidate_keys) != expected_candidate_rows
    ):
        raise ValueError("v9 candidate-5 reuse rows are incomplete or duplicated")

    record: dict[str, Any] = {
        "screen_name": selection["screen_name"],
        "selection_status": selection["selection_status"],
        "selection_path": str(selection_path),
        "selection_file_sha256": EXPECTED_V9_SELECTION_FILE_SHA256,
        "design_selection_sha256": EXPECTED_V9_SELECTION_SHA256,
        "spec_path": str(spec_path),
        "spec_file_sha256": EXPECTED_V9_SPEC_FILE_SHA256,
        "design_spec_sha256": EXPECTED_V9_SPEC_SHA256,
        ...
        "v8_design_selection_sha256": selection[
            "v8_design_selection_sha256"
        ],
        "episode_metrics_sha256": EXPECTED_V9_EPISODE_SHA256,
        "cell_gates_sha256": EXPECTED_V9_GATE_SHA256,
        "new_shard_count": len(shard_index),
        "new_shard_row_count": len(new_rows),
        "new_shards": shard_index,
        "combined_row_count": len(combined_rows),
        "combined_rows_reconstructed_exactly": True,
        "reuse_candidate_index": V9_REUSE_CANDIDATE_INDEX,
        "reuse_candidate_id": v9.candidate_id(
            v9.DESIGN_GRID[V9_REUSE_CANDIDATE_INDEX]
        ),
        "reuse_policy_name": source_policy,
        "reuse_row_count": len(candidate_rows),
        "all_cells_pass": False,
        "fresh_validation_allowed": False,
        "fresh_test_allowed": False,
        "test_allowed": False,
    }
    record["v9_failure_audit_sha256"] = source_runner.sha256_json(record)
    return record, candidate_rows


def reuse_v9_control_rows(
    source_rows: Sequence[EpisodeMetrics],
) -> list[EpisodeMetrics]:
    source_candidate = v9.DESIGN_GRID[V9_REUSE_CANDIDATE_INDEX]
    ...
    for field in (
        ...
        if float(target[field]) != float(source_candidate[field]):
            raise RuntimeError(f"v9 control mapping changed: {field}")
    if (
        float(target["margin_cap"]) != MAX_EQUIVALENT_MARGIN_CAP
        or float(target["margin_cap"]) != MARGIN_CAPS[0]
    ):
        raise RuntimeError("v10 reuse margin cap is not action-equivalent")
    source_policy = v9.candidate_policy_name(source_candidate)
    if any(row.policy != source_policy for row in source_rows):
        raise ValueError("v9 reuse rows contain another policy")
...
def build_spec(
    ...
    baseline: Mapping[str, Any],
    v9_audit: Mapping[str, Any],
    *,
    runtime_code_fingerprint: Mapping[str, str] | None = None,
) -> dict[str, Any]:
    protocol = args.project.parent / "docs" / "MARGIN_CAPPED_SHIELD_DESIGN_V10.md"
    fingerprint = (
    ...
    body: dict[str, Any] = {
        ...
        "reused_logical_jobs": len(build_design_jobs())
        - len(build_new_evaluation_jobs()),
        ...
        "v9_failure_audit": dict(v9_audit),
        "feature_contract": {
        ...
        "paths": {
            ...
            "v9_output": str(args.v9_output.resolve()),
...
def _expected_shard_metadata(
    ...
    return {
        ...
        "v9_design_selection_sha256": spec["v9_failure_audit"][
            "design_selection_sha256"
        ],
        "v9_failure_audit_sha256": spec["v9_failure_audit"][
...
def _load_valid_shard(
    ...
    if not csv_path.is_file() or not metadata_path.is_file():
        raise FileNotFoundError(f"missing v10 shard: {job.job_id}")
    metadata = source_runner._load_json(metadata_path)
    ...
    if set(metadata) != required_fields:
        raise ValueError(f"v10 shard metadata field set mismatch: {job.job_id}")
    for field, value in expected.items():
        if metadata.get(field) != value:
            raise ValueError(f"v10 shard metadata mismatch: {job.job_id}/{field}")
    if metadata.get("csv_sha256") != sha256_file(csv_path):
        raise ValueError(f"v10 shard hash mismatch: {job.job_id}")
    if metadata.get("row_count") != len(DESIGN_WORKLOAD_SEEDS):
        raise ValueError(f"v10 shard row count mismatch: {job.job_id}")
    if not isinstance(metadata.get("policy_diagnostics"), Mapping):
        raise ValueError(f"v10 shard diagnostics missing: {job.job_id}")
...
def evaluate_job(
    ...
    if job.candidate_index == 0:
        raise RuntimeError("v9 control candidate must be reused, not evaluated")
    ...
    if csv_path.exists() or metadata_path.exists():
        if not csv_path.is_file() or not metadata_path.is_file():
            raise ValueError(f"partial v10 shard exists: {job.job_id}")
    ...
    policy, _ = load_margin_capped_hysteresis_policy(
        checkpoint_path,
        **job.as_parameters(),
        device=args.device,
    )
    if policy.checkpoint_schema != expected["policy_schema"]:
        raise ValueError(f"loaded v10 policy schema mismatch: {job.job_id}")
...
def evaluate_jobs(
    ...
    if len(rows) != expected_count or len(keys) != expected_count:
        raise RuntimeError("v10 new-evaluation grid is incomplete or duplicated")
    return rows
...
def compute_candidate_cell_gates(
                ...
                gates.append(
                    {
                        **dict(candidate),
                        ...
                        "shield_switch_opportunities_total": (
                            shield_opportunities
                        ),
    ...
    if len(gates) != expected:
        raise RuntimeError("v10 gate grid has the wrong size")
    return gates
...
def select_candidate(
    ...
    if selected is not None:
        ...
        if len(tied) != 1:
            raise RuntimeError("v10 selection argmax is not unique")
...
def _validate_artifact(
    ...
    if path != expected_path.resolve() or not path.is_file():
        raise ValueError(f"v10 artifact path mismatch: {expected_path.name}")
    if artifact.get("sha256") != sha256_file(path):
        raise ValueError(f"v10 artifact hash mismatch: {expected_path.name}")
    if artifact.get("row_count") != expected_rows:
        raise ValueError(f"v10 artifact row count mismatch: {expected_path.name}")
    if path.suffix.lower() == ".csv":
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            observed_rows = sum(1 for _ in csv.DictReader(handle))
        if observed_rows != expected_rows:
            raise ValueError(
                f"v10 CSV physical row count mismatch: {expected_path.name}"
            )
...
def _csv_rows_match(
    ...
    if observed != rendered:
        raise ValueError(f"v10 CSV does not replay: {path.name}")
    ...
    body: dict[str, Any] = {
        ...
        "artifacts": {
            "design_spec": _artifact(
                output / "margin_capped_shield_design_spec.json", 1
            ),
...
def write_design_artifacts(
    ...
    aggregate = source_runner.aggregate_rows(rows, rng_base=81000)
...
def load_design_selection(
    ...
    *,
    ...
    if (
        ...
        and sha256_file(path) != EXPECTED_V10_SELECTION_FILE_SHA256
    ):
    ...
    if (
        allow_historical_runtime
        and selection_sha256 != EXPECTED_V10_SELECTION_SHA256
    ):
        raise ValueError("unexpected v10 design selection")
    if selection.get("screen_name") != SCREEN_NAME:
        raise ValueError("v10 selection screen mismatch")
    if selection.get("selection_rule_id") != SELECTION_RULE_ID:
        raise ValueError("v10 selection rule mismatch")
    for field in (
        "confirmatory",
        "paper_claim_allowed",
        "promotion_decision_allowed",
        ...
        if selection.get(field) is not False:
            raise ValueError(f"v10 selection must set {field}=false")
    if selection.get("candidate_grid") != [
        dict(candidate) for candidate in DESIGN_GRID
    ]:
        raise ValueError("v10 selection candidate grid mismatch")

    output = path.parent
    expected_paths = {
        "design_spec": output / "margin_capped_shield_design_spec.json",
    ...
    if not isinstance(artifacts, Mapping) or set(artifacts) != set(expected_paths):
        raise ValueError("v10 selection artifact mapping mismatch")

    spec_path = expected_paths["design_spec"]
    if (
        allow_historical_runtime
        and sha256_file(spec_path) != EXPECTED_V10_SPEC_FILE_SHA256
    ):
        raise ValueError("frozen v10 design specification changed")
    spec = source_runner._load_json(spec_path)
    spec_sha256 = _self_hash(spec, "design_spec_sha256")
    if allow_historical_runtime and spec_sha256 != EXPECTED_V10_SPEC_SHA256:
        raise ValueError("unexpected v10 design specification")
    if selection.get("design_spec_sha256") != spec["design_spec_sha256"]:
        raise ValueError("v10 selection spec mismatch")
    ...
    args = argparse.Namespace(
        ...
        v9_output=Path(str(spec["paths"]["v9_output"])).resolve(),
        output=output,
    ...
    if allow_historical_runtime:
        ...
        if fingerprint != spec.get("runtime_code_fingerprint"):
            raise ValueError("v10 runtime code fingerprint changed")
    ...
    if spec.get("source_runtime_equivalence") != source_runtime_record:
        raise ValueError("v10 source/runtime equivalence binding changed")
    ...
    if spec.get("v7_baseline") != expected_baseline:
        raise ValueError("v10 v7 baseline binding changed")
    v9_audit, source_control_rows = audit_v9_failure_and_shards(
        args.v9_output, source, baseline
    )
    if spec.get("v9_failure_audit") != v9_audit:
        raise ValueError("v10 v9 failure audit binding changed")
    ...
    if spec != expected_spec:
        raise ValueError("v10 design specification does not replay exactly")
    ...
    for field, expected_value in expected_selection_bindings.items():
        if selection.get(field) != expected_value:
            raise ValueError(f"v10 selection binding changed: {field}")
    reused_rows = reuse_v9_control_rows(source_control_rows)
    ...
    if selection.get("new_evaluation_shards") != expected_shards:
        raise ValueError("v10 new shard index does not replay")
    ...
    if observed_rows != rows:
        raise ValueError("v10 combined rows do not replay from sources")
    expected_plan = [job.as_dict() for job in build_design_jobs()]
    expected_aggregate = source_runner.aggregate_rows(rows, rng_base=81000)
    gates = compute_candidate_cell_gates(rows)
    ...
    if selection.get("candidate_results") != results:
        raise ValueError("v10 candidate results do not replay")
    eligible_indices = [
        int(result["candidate_index"])
        for result in results
        if result["eligible"]
    ...
    if selection.get("eligible_candidate_indices") != eligible_indices:
        raise ValueError("v10 eligible set does not replay")
    ...
    if selection.get("selected_candidate_index") != selected_index:
        raise ValueError("v10 selected candidate does not replay")
    selected_parameters = (
        {
            key: selected[key]
            for key in (
    ...
    if selection.get("selected_parameters") != selected_parameters:
        raise ValueError("v10 selected parameters do not replay")
    if selection.get("all_cells_pass") is not (selected is not None):
        raise ValueError("v10 all-cells status does not replay")
    expected_status = (
        "selected_exposed_design_candidate"
        if selected is not None
        else "no_eligible_candidate"
    ...
    if selection.get("selection_status") != expected_status:
        raise ValueError("v10 selection status does not replay")
    ...
    for name, expected_path in expected_paths.items():
        _validate_artifact(
            artifacts[name], expected_path, expected_row_counts[name]
        )
    ...
    if selection != expected_selection:
        raise ValueError("v10 design selection does not replay exactly")
    return selection
...
def validate_environment(args: argparse.Namespace) -> None:
    ...
    broad = {
        ...
        args.v9_output.resolve(),
        args.source_runtime_equivalence.parent.resolve(),
    }
    if args.output.resolve() in broad:
        raise RuntimeError("output must be a dedicated v10 design directory")
    if args.output.is_dir() and not (
        args.output / "margin_capped_shield_design_spec.json"
    ).is_file():
        if any(args.output.iterdir()):
            raise RuntimeError("output contains an unrecognized experiment")
    if (
        MARGIN_CAPS != (0.750, 0.375, 0.250)
        or len(DESIGN_GRID) != 3
        or len(build_design_jobs()) != 24
        or len(build_new_evaluation_jobs()) != 16
    ):
        raise RuntimeError("frozen v10 design grid changed")
    if DESIGN_WORKLOAD_SEEDS != tuple(range(60001, 60011)):
        raise RuntimeError("v10 must use only the exposed v7 design panel")
    if MARGIN_CAPS[0] != MAX_EQUIVALENT_MARGIN_CAP:
        raise RuntimeError("v10 row-reuse control changed")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Design the margin-capped shield on an exposed panel."
    )
    parser.add_argument(
        "--source",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Full semantic replay precedes every upstream artifact read and every write.
    source_runtime = load_source_runtime_equivalence(args)
    source = source_runner.audit_source_screen(args.source)
    ...
    reused_rows = reuse_v9_control_rows(source_control_rows)
    spec = build_spec(args, source, source_runtime, baseline, v9_audit)
    if args.dry_run:
        print(
            json.dumps(
                {
                    ...
                    "v9_new_shards_audited": v9_audit["new_shard_count"],
                    "v9_combined_rows_reconstructed_exactly": v9_audit[
                        "combined_rows_reconstructed_exactly"
                    ],
                    "candidate_count": len(DESIGN_GRID),
                    "logical_design_jobs": len(build_design_jobs()),
                    "new_evaluation_jobs": len(build_new_evaluation_jobs()),
    ...
    with source_runner.invocation_lock(args.output):
        if selection_path.exists():
            if not selection_path.is_file():
                raise ValueError("v10 design selection is not a file")
            selection = load_design_selection(selection_path)
        else:
            source_runner.ensure_immutable_json(
                args.output / "margin_capped_shield_design_spec.json", spec
            )
            atomic_write_csv(
