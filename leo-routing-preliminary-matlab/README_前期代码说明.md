...
因此 `leo_marl_env.py` 里预留了 `orbital_geodetic_coord()`。它现在只是 toy 版本的 `(U,W)` 特征占位，不是完整 TLE/SGP4。之后如果接 Hypatia、StarryNet、TLE 或其他真实星座数据，应通过 `topology_provider(t, env)` 替换 `_build_topology()`，并把真实的 shell/orbit/geographic 坐标填入观测。
...
为了后续把 toy topology 升级到真实/准真实星座拓扑，这次还补了了：
