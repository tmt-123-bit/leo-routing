...
%
% NOTE (updated 2026-08-12): Table I refreshed to the no_lifetime headline run
% (experiments/IEEE-NOLIFE-full). no_lifetime STRENGTHENS the headline: MAPPO
% now BEATS the Dijkstra/ECMP oracle on delivery in medium_load (+4.8pp,
% p=1.1e-9), fault_links (+4.3pp, p=1.2e-9) and frequent_break (+2.5pp,
% p=1.1e-6); ties low_load (dz=-0.23, p=0.051); trails only hotspot_high_load
% (-1.1pp). ALL PROSE IS UNCHANGED (paper parked; user declined a prose pass):
% the abstract, contributions, and Results paragraphs (incl. inline decimals,
% the C1-C5 "X pp" magnitudes, and the C2 claim that frequent_break is a TIE)
% still reflect the full-variant run and are therefore STALE / UNDERSTATED.
% The C2 paragraph cannot be mechanically number-refreshed because no_lifetime
% flips its qualitative claim (frequent_break: tie -> significant MAPPO win),
% which is a prose change. Treat Table I as authoritative current data; all
% prose awaits a future pass IF the paper is revived.
% =============================================================================
...
\multirow{4}{*}{\texttt{low\_load}}
 & \textbf{MAPPO (ours)} & \textbf{0.904} & \textbf{0.003} & \textbf{6.61} & \textbf{0.27} & \textbf{1.4} \\
...
\multirow{4}{*}{\texttt{medium\_load}}
 & \textbf{MAPPO (ours)} & \textbf{0.774} & \textbf{0.007} & \textbf{10.57} & \textbf{1.28} & 31.0 \\
...
\multirow{4}{*}{\texttt{hotspot\_high\_load}}
 & \textbf{MAPPO (ours)} & \textbf{0.284} & \textbf{0.168} & \textbf{20.66} & \textbf{6.82} & 118.5 \\
...
\multirow{4}{*}{\texttt{frequent\_break}}
 & \textbf{MAPPO (ours)} & \textbf{0.439} & \textbf{0.225} & \textbf{15.87} & \textbf{1.84} & \textbf{63.1} \\
...
\multirow{4}{*}{\texttt{fault\_links}}
 & \textbf{MAPPO (ours)} & \textbf{0.747} & \textbf{0.017} & \textbf{11.13} & \textbf{1.36} & 41.0 \\
