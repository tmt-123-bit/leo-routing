...
## 6. 泛化能力(零样本迁移,遮罩平价修复后)

| 实验 | 设置 | 结果 |
|---|---|---|
| **星座规模** | n24 训练 → n24/n66/n110/n132 零样本 | 吞吐比 **1.069***/**1.022**\*\*/1.011(平局,p=0.17)/**0.921**\*\*\* —— 4.6× 内持平或更好,5.5× 优雅退化 |
...
> 真实拓扑上 oracle 在原始投递率上仍领先(真实星间几何直径更长 → 更多在途包,这是 episode 长度的尺度塌缩伪影,非策略失败);**吞吐比(尺度不变)是诚实指标**,~93%,且 MAPPO 延迟与公平性显著更优。

数据:[`IEEE-SCALE-EVALFIX/`](experiments/IEEE-SCALE-EVALFIX/)、[`IEEE-LOAD-EVALFIX/`](experiments/IEEE-LOAD-EVALFIX/)、[`IEEE-FAULT-EVALFIX/`](experiments/IEEE-FAULT-EVALFIX/)、[`IEEE-REALISM-EVALFIX/`](experiments/IEEE-REALISM-EVALFIX/)。

## 7. 鲁棒性

- **收敛/可复现**:8 个训练 seed,最终回报 CV 1–9%;价值函数 EV 0.25–0.99。见 `figures/fig_convergence`。
- **奖励权重敏感**:7 个 OAT 配置(扰动 w_deliver/w_load/w_switch)全在 baseline ±1.5 pp 内,每个仍领先 Dijkstra(遮罩平价修复后)3.9–6.0 pp —— 拥塞感知优势不是某个特定权重调出来的。
